...
async def main_async(profile=None):
    ...
    # 1. 用firecrawl_client深度调研抓取市场综述
    client = FirecrawlClient(FirecrawlConfig())
    await client.initialize()
    result = await client.debug_deep_research(
        query="请对今天的全球金融市场热点新闻进行深度调研和总结，输出一份简明扼要的市场综述（300字以内），涵盖主要事件、影响和市场情绪。",
        maxDepth=2,
        timeLimit=60,
        maxUrls=10
    )
    ...
    logs.append({"step": "raw_content", "desc": "MCP原始市场内容", "content": raw_content[:500]})

    # 3. 用大模型做市场综述摘要
    summary = await get_market_summary_via_deep_research(raw_content, model)
    ...
    # 5. 用summary作为新闻，sentiment作为情绪值，进入分步BDI+博弈流程
    news = summary
